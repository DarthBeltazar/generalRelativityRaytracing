        ...
        threads.push_back(std::thread([yaw, pitch, start, end, h, rs, width, widthd, heightd, aspect, &bhpos, &output]()-> void {
            for (int y = start; y < end; y++) {
                for (int x = 0; x < width; x++) {
                    Ray ray = generateRay(x, y, widthd, heightd, aspect, Vec3(), yaw, pitch);
...
void writeImage(std::vector<HitInfo> his, const int WIDTH, const int HEIGHT, const char *filename, double time, const Background &background) {
...
void renderImage(const int WIDTH, const int HEIGHT, const char *filename, const double h, const double rs, const Vec3 &bhpos, const double yaw, const double pitch, double time, const Background &background) {
